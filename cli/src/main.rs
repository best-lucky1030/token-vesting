...
use token_vesting::instruction::{create, unlock, VestingInstruction};
...
fn command_create_svc(
    ...
    let mut transaction =
        Transaction::new_with_payer(&instructions, Some(&source_token_owner.pubkey()));
...
fn command_unlock_svc(
    ...
    let mut transaction =
        Transaction::new_with_payer(&[unlock_instruction], Some(&source_pubkey.pubkey()));
...
fn main() {
    let matches = App::new(crate_name!())
        ...
        .subcommand(
            SubCommand::with_name("create-svc")
        ...
        .subcommand(
            SubCommand::with_name("unlock-svc")
                .about("Unlock a simple vesting contract")
                .arg(
                    Arg::with_name("seed")
                        .long("seed")
                        .value_name("ADDRESS")
    ...
    let _ = match matches.subcommand() {
        ("create-svc", Some(arg_matches)) => {
            let vesting_seed = (*String::as_bytes(&value_of(arg_matches, &"seed").unwrap()))
                .try_into()
                .unwrap();
