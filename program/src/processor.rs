...
use crate::{error::VestingError, instruction::VestingInstruction};
...
impl Processor {
    pub fn process_lock(
        ...
        **source_account.try_borrow_mut_lamports()? -= amount;
        ...
        invoke_signed(
            ...
            &[self_account.clone(), program_account.clone()],
            &[&[&[42, 42]]],
        );
        Ok(())
    }
    ...
    pub fn process_unlock(_program_id: &Pubkey, _accounts: &[AccountInfo]) -> ProgramResult {
        ...
        if clock.slot > 3 {
            //TODO get the slot_height from the Vesting Schedule (master) contract
            **receiver_account.try_borrow_mut_lamports()? +=
                **programm_account.try_borrow_mut_lamports()?;
